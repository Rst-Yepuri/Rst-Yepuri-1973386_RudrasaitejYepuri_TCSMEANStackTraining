function totalQuantity(): number {
    ...
    if (sessionStorage.length == 0) {
        ...
        for (let i = 0; i < sessionStorage.length; i++) {
            let x = JSON.parse(sessionStorage.getItem(sessionStorage.key(i)!)!);
            result += x.quantity;
        }
    }
    return result;
...
function price(quantity:number, price:string): number{
...
function checkOut() {
    ...
    let mysubtotal = document.getElementById('subtotal') as HTMLInputElement;
    let mytax = document.getElementById('tax') as HTMLInputElement;
    let mytotal = document.getElementById('total') as HTMLInputElement;
...
function storeToSession(e: string): void {
    const div_element = document.getElementById(e);
    ...
    if (sessionStorage.getItem(e)) {
        ...
        newProduct.quantity = <any>newProduct.quantity + 1;
